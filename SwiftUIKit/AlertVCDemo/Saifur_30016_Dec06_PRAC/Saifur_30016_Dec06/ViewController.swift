...
extension ViewController:UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            cell2.imgView.image = UIImage(named: Car.car[indexPath.row].image)
...
//        performSegue(withIdentifier: Constant.segueWindow, sender: nil)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender:Any?){
        if segue.identifier == "segueNew" {
            if let second = segue.destination as? NewViewController {
               // second.deleGete = self
                let index = tableView.indexPathForSelectedRow

                second.forImage = Car.car[index!.row].image //"Toyota"
                second.forName = Car.car[index!.row].name //"DemoText1"
                second.forDesc = Car.car[index!.row].description //"this is desc"
            }}}
